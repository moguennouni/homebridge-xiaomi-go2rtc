...
class Go2rtc {
  ...
  async ensureBinary() {
    ...
    if (this.binaryPath) {
      if (!fs.existsSync(this.binaryPath)) {
        throw new Error(`go2rtc introuvable à l'emplacement configuré : ${this.binaryPath}`);
      }
      this.variant = 'custom';
    ...
    if (!this.useOfficial) {
      ...
      this.log.warn(`go2rtc-xiaomi-control n'est pas disponible pour ${key} : utilisation du go2rtc officiel, sans orientation, réglages ni correction du son à 16 kHz.`);
    }

    const asset = GO2RTC_ASSETS[key];
    if (!asset) {
      throw new Error(`Aucun binaire go2rtc connu pour ${key}. Installez go2rtc vous-même et renseignez "go2rtcPath".`);
    }
  ...
  async download({ name, url, sha256, target }) {
    ...
    this.log.info(`Téléchargement de ${name}...`);
    const response = await fetch(url);
    if (!response.ok) {
      throw new Error(`Échec du téléchargement de ${name} (${response.status} ${response.statusText})`);
    }

    const data = Buffer.from(await response.arrayBuffer());
    const digest = crypto.createHash('sha256').update(data).digest('hex');
    if (digest !== sha256) {
      throw new Error(`Empreinte SHA-256 de ${name} invalide (attendu ${sha256}, obtenu ${digest}). Téléchargement refusé.`);
    }

    const tmp = `${target}.download`;
    await fsp.writeFile(tmp, data, { mode: 0o755 });
    await fsp.rename(tmp, target);
    this.log.info(`${name} téléchargé et vérifié.`);
  ...
  async killLeftover() {
    ...
    try {
      ...
      this.log.warn(`Arrêt d'une ancienne instance de go2rtc (pid ${pid}).`);
  ...
  async start() {
    ...
    child.on('error', (err) => this.log.error(`Impossible de lancer go2rtc : ${err.message}`));
    child.on('exit', (code, signal) => {
      ...
      this.log.warn(`go2rtc s'est arrêté (code ${code}, signal ${signal}). Redémarrage dans ${RESTART_DELAY_MS / 1000} s.`);
      this.restartTimer = setTimeout(() => {
        this.start().catch((err) => this.log.error(`Redémarrage de go2rtc impossible : ${err.message}`));
      }, RESTART_DELAY_MS);
    });
  ...
  async waitReady(timeoutMs = 20000) {
    ...
    throw new Error(`go2rtc ne répond pas sur le port ${this.apiPort}`);
  }
  ...
  async setProperty(streamName, siid, piid, value) {
    ...
    if (failed) {
      throw new Error(`refusé par le cloud Xiaomi (code ${failed.code})`);
    }
  }
