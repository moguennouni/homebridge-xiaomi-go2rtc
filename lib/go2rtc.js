...
// Transcoding shared by every viewer of a camera (go2rtc source `ffmpeg:<stream>#video=homekit_h264`): ultrafast
// holds real time on small ARM boards, and a key frame every 2 s lets a new viewer start quickly.
// dump_extra repeats SPS/PPS before each key frame: the RTSP muxer asks x264 for global headers only, and HomeKit
// cannot decode a copied stream without them
const SHARED_H264_TEMPLATE_NAME = 'homekit_h264';
const SHARED_H264_TEMPLATE = '-c:v libx264 -preset:v ultrafast -tune:v zerolatency -profile:v high -level:v 4.0 '
  + '-pix_fmt:v yuv420p -bf 0 -bsf:v dump_extra -force_key_frames expr:gte(t,n_forced*2) -b:v 600k -maxrate 600k -bufsize 1200k';
...
class Go2rtc {
  constructor({ log, storagePath, apiPort, rtspPort, binaryPath, useOfficial, ffmpegPath, username, password }) {
    ...
    // (url, line) => void, called when go2rtc loses the connection to a camera
  ...
  async writeConfig() {
    ...
    config.ffmpeg = { ...config.ffmpeg, bin: this.ffmpegPath, [SHARED_H264_TEMPLATE_NAME]: SHARED_H264_TEMPLATE };
    ...
    const onOutput = (chunk) => {
      for (const rawLine of chunk.toString().split('\n')) {
        ...
        if (/ (ERR|FTL) /.test(line)) {
          ...
          // e.g. `producer.go:170 > error="miss: read media: ... i/o timeout" url=xiaomi://...`
          const lost = this.onSourceLost && /producer\.go/.test(line) && line.match(/url=(xiaomi:\/\/\S+)/);
          if (lost) {
            this.onSourceLost(lost[1], line);
          }
        } else {
  ...
  // Like a Homebridge restart for go2rtc: every camera connection and transcoding starts over
  async restart() {
    this.stop();
  ...
  // Events the camera uploaded to the Xiaomi cloud, newest first (patched go2rtc): [{ eventType, createTime, fileId }]
  async listEvents(streamName, { minutes, limit }) {
    const query = new URLSearchParams({ src: streamName, minutes: String(minutes), limit: String(limit) });
    const result = await this.request(`/api/xiaomi/events?${query}`);
    if (result && result.code) {
      throw new Error(t('settingsRefused', { code: result.code }));
    }
    return (result && result.data && result.data.thirdPartPlayUnits) || [];
  }

  // Two-way audio (patched go2rtc): raw A-law, 16 kHz, mono, written to the returned request plays on the camera
  // speaker until the request ends. At 8 kHz, the tested camera played the voice an octave higher with gaps.
  openSpeaker(streamName) {
    const query = new URLSearchParams({ src: streamName, format: 'pcma', rate: '16000' });
    return http.request(`${this.apiUrl}/api/xiaomi/speaker?${query}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/octet-stream' },
    });
  }

  // JPEG thumbnail the camera took with a cloud event (patched go2rtc)
  async getEventImage(streamName, fileId, storeId, { signal } = {}) {
    const query = new URLSearchParams({ src: streamName, fileId: String(fileId), stoId: storeId });
    const response = await fetch(`${this.apiUrl}/api/xiaomi/image?${query}`, { signal });
    if (!response.ok) {
      const error = new Error(`go2rtc /api/xiaomi/image : ${response.status} ${(await response.text()).trim()}`);
      error.status = response.status;
      throw error;
    }
  ...
  // Server-Sent Events of what the camera says (patched go2rtc). A camera with motion tracking enabled sends
  // a motor message on its own when it turns to follow a movement.
  async watchMotion(streamName, { signal, onReady, onMessage }) {
    const query = new URLSearchParams({ src: streamName });
    const response = await fetch(`${this.apiUrl}/api/xiaomi/motion?${query}`, { signal });
    if (!response.ok) {
      const error = new Error(`go2rtc /api/xiaomi/motion : ${response.status} ${(await response.text()).trim()}`);
      error.status = response.status;
      throw error;
    }
  ...
  // A H.264 copy of a stream, transcoded by go2rtc only while someone reads it, whatever the number of readers
  async setSharedH264Stream(name, sourceStreamName) {
    await this.setStream(name, `ffmpeg:${sourceStreamName}#video=${SHARED_H264_TEMPLATE_NAME}#audio=copy`);
  }
