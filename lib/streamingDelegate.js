...
class StreamingDelegate {
    ...
    if (!this.probing) {
      this.probing = ffmpeg.probe(this.ffmpegPath, this.url)
        .then((codecs) => {
          ...
          this.log.info(this.prefix(`Flux détecté : vidéo ${codecs.video}, audio ${codecs.audio || 'aucun'} → ${copy ? 'copie directe' : 'réencodage en H.264'}.`));
          return codecs;
        })
        .finally(() => {
          this.probing = null;
  ...
  handleSnapshotRequest(request, callback) {
    this.getSnapshot(request.width)
      ...
      .catch((err) => {
        this.log.warn(this.prefix(`Capture d'écran impossible : ${err.message}`));
        callback(err);
      });
  }
  ...
  async prepareStream(request, callback) {
    try {
      ...
      this.log.error(this.prefix(`Préparation du flux impossible : ${err.message}`));
      callback(err);
    }
  }
    ...
    switch (request.type) {
      case StreamRequestTypes.START:
        this.startStream(request)
          .then(() => callback())
          .catch((err) => {
            this.log.error(this.prefix(`Démarrage du flux impossible : ${err.message}`));
            callback(err);
          });
        break;
      case StreamRequestTypes.RECONFIGURE:
        this.log.debug(this.prefix(`Reconfiguration demandée (${request.video.width}x${request.video.height}) ignorée.`));
        callback();
        break;
  ...
  buildArgs(session, request, codecs) {
    ...
    return { args, description: `${copy ? 'copie' : `réencodage ${Math.min(width, this.camera.maxWidth || width)}px`} ${width}x${height}@${fps} ${bitrate} kbit/s${withAudio ? ' + audio' : ''}` };
  }
  ...
  async startStream(request) {
    ...
    try {
      ...
      if (this.camera.videoMode === 'auto') {
        throw new Error(`impossible de lire le flux go2rtc (${err.message})`);
      }
    ...
    this.log.info(this.prefix(`Démarrage du flux (${description}).`));
    ...
    const armTimeout = (ms) => {
      ...
      active.timer = setTimeout(() => {
        this.log.info(this.prefix('Plus de réponse de l\'appareil qui regarde, arrêt du flux.'));
    ...
    child.on('error', (err) => this.log.error(this.prefix(`Impossible de lancer ffmpeg (${this.ffmpegPath}) : ${err.message}`)));
    child.on('exit', (code, signal) => {
      if (active.stopping) {
        return;
      }
      this.log.error(this.prefix(`ffmpeg s'est arrêté (code ${code}, signal ${signal}) : ${active.lastLines.join(' | ') || 'aucun message'}`));
      this.controller.forceStopStreamingSession(sessionID);
      this.stopStream(sessionID);
    });
  }
  ...
  stopStream(sessionID) {
    ...
    this.log.info(this.prefix('Flux arrêté.'));
  }
