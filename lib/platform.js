...
class XiaomiGo2rtcPlatform {
  constructor(log, config, api) {
    ...
    api.on('didFinishLaunching', () => {
      this.start().catch((err) => this.log.error(`Démarrage impossible : ${err.message}`));
    });
    api.on('shutdown', () => {
  ...
  async start() {
    ...
    this.log.debug(`ffmpeg utilisé : ${this.ffmpegPath}`);
    ...
    const variants = { 'xiaomi-control': 'go2rtc-xiaomi-control', official: 'go2rtc officiel', custom: `go2rtc personnalisé (${config.go2rtcPath})` };
    this.log.info(`${variants[this.go2rtc.variant]} démarré. Interface web : ${this.uiUrl}`);
    if (!(config.uiUsername && config.uiPassword)) {
      this.log.warn('L\'interface go2rtc n\'a pas de mot de passe : tout appareil du réseau local peut voir vos caméras. Renseignez "uiUsername" et "uiPassword" dans la configuration du plugin.');
    }
  ...
  async discover() {
    try {
      ...
      this.log.warn(`Recherche des caméras Xiaomi impossible : ${err.message}`);
    }
  ...
  async findCameras() {
    ...
    if (!accounts.length) {
      if (!this.accountHintShown) {
        this.log.info(`Aucun compte Xiaomi connecté. Ouvrez ${this.uiUrl}, cliquez sur "Add" puis "Xiaomi" et connectez-vous : vos caméras apparaîtront ici automatiquement.`);
    ...
    for (const account of accounts) {
      ...
      for (const region of regions) {
        ...
        try {
          ...
          lastError = err;
          this.log.debug(`Compte ${account}, région ${region} : ${err.message}`);
          continue;
        }
        if (sources.length) {
      ...
      if (regionsWithCameras.length) {
        ...
        this.log.warn(`Compte Xiaomi ${account} : ${lastError.message}`);
      } else if (!this.cameras.size && !this.noCameraHintShown) {
        this.log.info(`Aucune caméra trouvée automatiquement sur le compte Xiaomi ${account}. Si la caméra est visible dans Mi Home, déclarez-la à la main (did + IP) dans "Réglages par caméra".`);
  ...
  async addOrUpdateCamera(source) {
    ...
    if (override.hidden) {
      if (!this.hiddenLogged.has(source.did)) {
        this.log.info(`Caméra "${source.name}" (${source.did}) masquée par la configuration.`);
    ...
    if (existing) {
      if (existing.url !== url) {
        ...
        existing.url = url;
        this.log.info(`[${existing.delegate.camera.name}] Adresse mise à jour (${source.ip}).`);
      }
      return;
    }
    ...
    const { hap } = this.api;
    const camera = {
      name: override.name || source.name || `Caméra ${source.did}`,
      streamName,
    ...
    this.log.info(`Caméra ajoutée : "${camera.name}" (${source.model}, ${source.ip}, did ${source.did}).`);
    this.log.info('Pour l\'ajouter à l\'app Maison : "Ajouter un accessoire" → "Autres options...", choisissez la caméra puis saisissez le code PIN de Homebridge.');

    // Probe now so the first stream in the Home app starts without the extra delay
    delegate.getCodecs().catch((err) => {
      this.log.warn(`[${camera.name}] Flux illisible pour l'instant : ${err.message}`);
    });
  }
  ...
  async addSettingsSwitches(accessory, camera, model) {
    ...
    try {
      ...
      this.log.warn(`[${camera.name}] Spécification du modèle ${model} indisponible : ${err.message}`);
      return [];
    }
    if (!spec) {
      this.log.warn(`[${camera.name}] Modèle "${model}" inconnu de miot-spec.org : renseignez le bon "Modèle Xiaomi" (par exemple chuangmi.camera.026c02) pour avoir les réglages.`);
      return [];
    }
    ...
    for (const control of controls) {
      ...
      on.onSet(async (value) => {
        ...
        try {
          ...
          this.log.info(`[${camera.name}] ${control.name} : ${enabled ? 'activé' : 'désactivé'}.`);
        } catch (err) {
          this.log.warn(`[${camera.name}] ${control.name} : modification impossible (${this.settingsError(err)}).`);
    ...
    const refresh = async () => {
      try {
        ...
        this.log.debug(`[${camera.name}] Lecture des réglages impossible : ${this.settingsError(err)}`);
      }
    };
    refresh();
    this.timers.push(setInterval(refresh, SETTINGS_REFRESH_MS));

    this.log.info(`[${camera.name}] Réglages disponibles : ${controls.map((c) => c.name).join(', ') || 'aucun'}.`);
    return services;
  }

  settingsError(err) {
    if (err.status === 404 && !/stream not found/.test(err.message)) {
      return 'ce go2rtc ne gère pas les réglages : il faut go2rtc-xiaomi-control, utilisé par défaut (retirez "go2rtcPath" et "useOfficialGo2rtc")';
    }
    return err.message;
  }
  ...
  addPtzButtons(accessory, camera, override) {
    ...
    const directions = [
      { id: 'left', name: 'Gauche', data: override.ptzLeft || '{"operation":2}', steps: panSteps },
      { id: 'right', name: 'Droite', data: override.ptzRight || '{"operation":1}', steps: panSteps },
      { id: 'up', name: 'Haut', data: override.ptzUp || '{"operation":4}', steps: tiltSteps },
      { id: 'down', name: 'Bas', data: override.ptzDown || '{"operation":3}', steps: tiltSteps },
    ];
  ...
  async moveCamera(camera, direction) {
    try {
      ...
      this.log.debug(`[${camera.name}] ${direction.name} ×${direction.steps}${position ? ` (position : angle ${position[1]}, inclinaison ${position[2]})` : ''}.`);
    } catch (err) {
      if (err.status === 409) {
        this.log.warn(`[${camera.name}] Ouvrez la caméra dans l'app Maison pour pouvoir l'orienter.`);
      } else if (err.status === 404 && !/stream not found/.test(err.message)) {
        this.log.warn(`[${camera.name}] Ce go2rtc ne gère pas l'orientation : il faut go2rtc-xiaomi-control, utilisé par défaut (retirez "go2rtcPath" et "useOfficialGo2rtc").`);
      } else {
        this.log.warn(`[${camera.name}] Orientation impossible : ${err.message}`);
      }
      throw err;
    }
  }
