...
// After a lost camera connection, go2rtc is restarted after this delay, and at most once per interval
const SOURCE_LOST_RESTART_DELAY_MS = 10 * 1000;
const SOURCE_LOST_MIN_INTERVAL_MS = 5 * 60 * 1000;
...
// The notification waits for the thumbnail at most this long
const EVENT_IMAGE_TIMEOUT_MS = 8 * 1000;
...
// HomeKit needs at least 4 s of recording prebuffer; beyond 30 s the memory use is not worth it
const prebufferSeconds = (value, fallback) => Math.min(Math.max(Number(value) || fallback, 4), 30);
...
class XiaomiGo2rtcPlatform {
  constructor(log, config, api) {
    ...
    this.timers = [];
    this.motionWatchers = [];
    this.go2rtcRestartTimer = null;
    this.lastGo2rtcRestart = 0;
    this.accounts = [];
    ...
    api.on('shutdown', () => {
      ...
      clearTimeout(this.go2rtcRestartTimer);
  ...
  async start() {
    ...
    this.go2rtc.onSourceLost = () => this.scheduleGo2rtcRestart();
  ...
  // go2rtc reconnects a lost camera by itself, but the shared transcoding then starts over under its readers: the
  // recording prebuffer keeps a stale initialization segment and the live view stops after a few seconds. Restarting
  // go2rtc, as a Homebridge restart does, gets everything back.
  scheduleGo2rtcRestart() {
    if (this.go2rtcRestartTimer || Date.now() - this.lastGo2rtcRestart < SOURCE_LOST_MIN_INTERVAL_MS) {
      return;
    }
    this.log.warn(t('sourceLost', { seconds: SOURCE_LOST_RESTART_DELAY_MS / 1000 }));
    this.go2rtcRestartTimer = setTimeout(async () => {
      this.go2rtcRestartTimer = null;
      this.lastGo2rtcRestart = Date.now();
      try {
        await this.go2rtc.restart();
        this.log.info(t('sourceLostRestarted'));
      } catch (err) {
        this.log.error(t('go2rtcRestartFailed', { error: err.message }));
      }
    }, SOURCE_LOST_RESTART_DELAY_MS);
  }
  ...
  async addOrUpdateCamera(source) {
    ...
    // One transcoding in go2rtc for every viewer, instead of one per viewer: see StreamingDelegate.buildArgs
    const sharedStreamName = override.sharedTranscode === false ? null : `${streamName}_h264`;
    if (sharedStreamName) {
      await this.go2rtc.setSharedH264Stream(sharedStreamName, streamName);
    }
    ...
    const camera = {
      ...
      recording: override.recording === true,
      recordingPrebufferMs: prebufferSeconds(override.recordingPrebuffer, 4) * 1000,
      recordingAnnouncedPrebufferMs: prebufferSeconds(override.recordingPrebufferAnnounced, override.recordingPrebuffer || 4) * 1000,
      // Experimental: the camera sometimes closes its connection while receiving audio
      twoWayAudio: override.twoWayAudio === true,
    ...
    // Without HomeKit Secure Video, HAP does not link the motion sensor: a notification would open another service
    if (delegate.controller.motionService) {
      extraServices.push(delegate.controller.motionService);
    }
    ...
    if (camera.motion) {
      ...
      this.log.warn(`[${camera.name}] ${t('recordingNeedsMotion')}`);
    }
  ...
  watchMotion(camera, delegate) {
    ...
    // The camera source can fire every few seconds, so only the rarer cloud detections are logged by default
    const detected = (message, level = 'debug') => {
      service.updateCharacteristic(hap.Characteristic.MotionDetected, true);
      this.log[level](`[${camera.name}] ${message}`);
    ...
    if (camera.motionSource === 'camera') {
      ...
      this.pollCloudMotion(camera, watcher.signal, detected, delegate);
    }
  }
  ...
  // A live snapshot would show the scene 20 to 30 s after the movement: the notification gets the thumbnail the
  // camera took with the event instead. The sensor still fires when the thumbnail cannot be read.
  async loadEventSnapshot(camera, delegate, event, signal) {
    if (!delegate || !event.fileId || !event.imgStoreId) {
      return;
    }
    try {
      const timeout = AbortSignal.any([signal, AbortSignal.timeout(EVENT_IMAGE_TIMEOUT_MS)]);
      delegate.setEventSnapshot(await this.go2rtc.getEventImage(camera.streamName, event.fileId, event.imgStoreId, { signal: timeout }));
    } catch (err) {
      if (!signal.aborted) {
        this.log.warn(`[${camera.name}] ${t('eventImageFailed', { error: err.message })}`);
      }
    }
  }
  ...
  async pollCloudMotion(camera, signal, detected, delegate) {
    ...
    while (!signal.aborted) {
      ...
      try {
        ...
        if (latest) {
          // Before the sensor fires: the Home app asks for the snapshot of the notification right away
          await this.loadEventSnapshot(camera, delegate, latest, signal);
          const delaySeconds = Math.round((now - latest.createTime) / 1000);
          detected(t('motionCloudDetected', { type: latest.eventType || '?', delay: delaySeconds }), 'info');
        }
      } catch (err) {
        if (signal.aborted) {
          return;
  ...
  addPtzButtons(accessory, camera, override) {
    ...
    const directions = [
      { id: 'left', name: t('ptzLeft'), data: override.ptzLeft || '{"operation":1}', steps: panSteps },
      { id: 'right', name: t('ptzRight'), data: override.ptzRight || '{"operation":2}', steps: panSteps },
      { id: 'up', name: t('ptzUp'), data: override.ptzUp || '{"operation":3}', steps: tiltSteps },
      { id: 'down', name: t('ptzDown'), data: override.ptzDown || '{"operation":4}', steps: tiltSteps },
    ];
