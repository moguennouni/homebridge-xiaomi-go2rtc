...
const run = (ffmpegPath, args, timeoutMs) => new Promise((resolve, reject) => {
  ...
  const timer = setTimeout(() => {
    child.kill('SIGKILL');
    reject(new Error(`ffmpeg n'a pas répondu en ${timeoutMs / 1000} s`));
  }, timeoutMs);
...
const probe = async (ffmpegPath, url, timeoutMs = 30000) => {
  ...
  if (!video) {
    ...
    throw new Error(`aucune piste vidéo trouvée (${lastLine})`);
  }
...
const snapshot = async (ffmpegPath, url, width, timeoutMs = 15000) => {
  ...
  if (code !== 0 || stdout.length === 0) {
    throw new Error(`capture impossible (${stderr.trim().split('\n').pop() || `code ${code}`})`);
  }
  return stdout;
};
